...
export async function getCachedTrendingStocks() {
  try {
    ...
    if (gainersError) {
      if (gainersError.message?.includes("does not exist") || gainersError.code === 'PGRST205' || gainersError.message?.includes("Could not find the table")) {
        return {
          top_gainers: [],
          top_losers: [],
          last_updated: null,
    ...
    if (losersError) {
      if (losersError.message?.includes("does not exist") || losersError.code === 'PGRST205' || losersError.message?.includes("Could not find the table")) {
        return {
          top_gainers: [],
          top_losers: [],
          last_updated: null,
...
export async function getCachedMostActiveStocks() {
  try {
    ...
    if (error) {
      if (error.message?.includes("does not exist") || error.code === 'PGRST205' || error.message?.includes("Could not find the table")) {
        return {
          stocks: [],
          last_updated: null,
          tablesNotExist: true,
